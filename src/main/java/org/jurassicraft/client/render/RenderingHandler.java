...
public enum RenderingHandler {
	...
	public static void onModelEvent(final ModelRegistryEvent event) {
		for (EnumDyeColor color : EnumDyeColor.values()) {
			registerItemRenderer(Item.getItemFromBlock(CULTIVATOR_BOTTOM), color.getMetadata(),
					"cultivate/cultivate_bottom_" + color.getName().toLowerCase(Locale.ENGLISH));
		}

		for (TreeType type : TreeType.values()) {
			ModelLoader.setCustomStateMapper(ANCIENT_FENCES.get(type), new MultipartStateMap());
			ModelLoader.setCustomStateMapper(ANCIENT_FENCE_GATES.get(type),
					new StateMap.Builder().ignore(BlockFenceGate.POWERED).build());
			ModelLoader.setCustomStateMapper(BlockHandler.ANCIENT_DOORS.get(type),
		...
		ModelLoader.setCustomStateMapper(STYLOPHYLLOPSIS,
				new StateMap.Builder().ignore(AncientCoralBlock.LEVEL).build());
		ModelLoader.setCustomStateMapper(HIPPURITES_RADIOSUS,
				new StateMap.Builder().ignore(AncientCoralBlock.LEVEL).build());
		...
		// ModelLoader.setCustomStateMapper(BlockHandler.MED_SECURITY_FENCE_BASE, new
		// MultipartStateMap());
		// ModelLoader.setCustomStateMapper(BlockHandler.MED_SECURITY_FENCE_POLE, new
		// MultipartStateMap());
		// ModelLoader.setCustomStateMapper(BlockHandler.MED_SECURITY_FENCE_WIRE, new
		// MultipartStateMap());
		// ModelLoader.setCustomStateMapper(BlockHandler.HIGH_SECURITY_FENCE_BASE, new
		// MultipartStateMap());
		// ModelLoader.setCustomStateMapper(BlockHandler.HIGH_SECURITY_FENCE_POLE, new
		// MultipartStateMap());
		// ModelLoader.setCustomStateMapper(BlockHandler.HIGH_SECURITY_FENCE_WIRE, new
		// MultipartStateMap());
		...
		/*
		 * for (EnumDyeColor color : EnumDyeColor.values()) {
		 * registerBlockRenderer(CULTIVATOR_BOTTOM, color.ordinal(),
		 * "cultivate/cultivate_bottom_" + color.getName().toLowerCase(Locale.ENGLISH));
		 * }
		 */
		...
		for (FossilizedTrackwayBlock.TrackwayType trackwayType : FossilizedTrackwayBlock.TrackwayType.values()) {
			registerBlockRenderer(FOSSILIZED_TRACKWAY, trackwayType.ordinal(),
					"fossilized_trackway_" + trackwayType.getName());
		}
		...
		for (AttractionSignEntity.AttractionSignType type : AttractionSignEntity.AttractionSignType.values()) {
			registerItemRenderer(ATTRACTION_SIGN, type.ordinal(),
					"attraction_sign_" + type.name().toLowerCase(Locale.ENGLISH));
		}
		...
		for (Dinosaur dinosaur : EntityHandler.getDinosaurs().values()) {
			...
			// registerItemRenderer(ACTION_FIGURE, meta, "action_figure/action_figure_" +
			// formattedName);
	...
	public void preInit() {
		...
		// registerRenderInfo(EntityHandler.ALLIGATORGAR, new AlligatorGarAnimator(),
		// 0.0F);
		...
		// registerRenderInfo(EntityHandler.STEGOSAURUS, new StegosaurusAnimator(),
		// 0.65F);
	...
	public void init() {
		...
		blockColors
				.registerBlockColorHandler(
						(state, access, pos,
								tintIndex) -> pos != null ? BiomeColorHelper.getGrassColorAtPos(access, pos) : 0xFFFFFF,
						MOSS);

		for (Map.Entry<TreeType, AncientLeavesBlock> entry : ANCIENT_LEAVES.entrySet()) {
			blockColors.registerBlockColorHandler(
					(state, access, pos, tintIndex) -> pos == null ? ColorizerFoliage.getFoliageColorBasic()
							: BiomeColorHelper.getFoliageColorAtPos(access, pos),
					entry.getValue());
		}
		...
		if (JurassiCraftConfig.VEHICLES.tourRailBlockEnabled)
			blockColors.registerBlockColorHandler((state, access, pos,
					tintIndex) -> tintIndex == 1 ? ((TourRailBlock) state.getBlock()).getSpeedType().getColor() : -1,
		...
		if (JurassiCraftConfig.VEHICLES.tourRailBlockEnabled)
			itemColors.registerItemColorHandler(
					(stack, tintIndex) -> tintIndex == 1
							? ((TourRailBlock) ((ItemBlock) stack.getItem()).getBlock()).getSpeedType().getColor()
							: -1,
					BlockHandler.TOUR_RAIL_SLOW, BlockHandler.TOUR_RAIL_MEDIUM, BlockHandler.TOUR_RAIL_FAST);

		for (Map.Entry<TreeType, AncientLeavesBlock> entry : ANCIENT_LEAVES.entrySet()) {
			itemColors.registerItemColorHandler((stack, tintIndex) -> ColorizerFoliage.getFoliageColorBasic(),
					entry.getValue());
		}
		...
		itemColors.registerItemColorHandler(
				((stack, tintIndex) -> tintIndex == 1 ? ((Dart) stack.getItem()).getDartColor(stack) : -1),
				DART_POISON_CYCASIN, DART_POISON_EXECUTIONER_CONCOCTION, DART_TIPPED_POTION, DART_TRANQUILIZER);
	}
	...
	public void postInit() {
		...
		ClientRegistry.bindTileEntitySpecialRenderer(ElectricFencePoleBlockEntity.class,
				new ElectricFencePoleRenderer());
		ClientRegistry.bindTileEntitySpecialRenderer(CleaningStationBlockEntity.class, new CleaningStationRenderer());
		// ClientRegistry.bindTileEntitySpecialRenderer(CultivatorBlockEntity.class, new
		// CultivatorRenderer());
	}
	...
	public static void registerItemRenderer(Item item, int meta, String path) {
		ModelLoader.setCustomModelResourceLocation(item, meta,
				new ModelResourceLocation(JurassiCraft.MODID + ":" + path, "inventory"));
	}
