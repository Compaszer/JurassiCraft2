...
public class Loot {
		...
		if (name.getResourceDomain().equals(JurassiCraft.MODID)) {
			if (name.getResourcePath().equals(Loot.VISITOR_GROUND_STORAGE.getResourcePath())) {
				...
				LootEntry tooth = Loot.entry(ItemHandler.FOSSILS.get("tooth")).weight(2).function(DINOSAUR_DATA)
						.count(1, 2).build();
				LootEntry actionFigure = Loot.entry(ItemHandler.DISPLAY_BLOCK).function(DINOSAUR_DATA).weight(1)
						.build();
	...
	public static class SpecifiedDinosaurData extends LootFunction {

		private int dinoId;

		public SpecifiedDinosaurData(int id) {
			super(new LootCondition[0]);
			dinoId = id;
		}

		public SpecifiedDinosaurData(LootCondition[] conditions) {
			super(conditions);
		}
		...
		public ItemStack apply(ItemStack stack, Random rand, LootContext context) {
			List<Dinosaur> dinosaurs = EntityHandler.getRegisteredDinosaurs();
			Dinosaur dinosaur = dinosaurs.get(dinoId);
			if (stack.getItem() instanceof FossilItem) {
				FossilItem s = (FossilItem) stack.getItem();
