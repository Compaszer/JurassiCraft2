...
public class ServerEventHandler {
	...
	public void decorate(DecorateBiomeEvent.Pre event) {
		...
		if (JurassiCraftConfig.MINERAL_GENERATION.plantFossilGeneration) {
			if (decorator != null && decorator.chunkProviderSettings != null
					&& !(decorator.coalGen instanceof WorldGenCoal)) {
				decorator.coalGen = new WorldGenCoal(Blocks.COAL_ORE.getDefaultState(),
						decorator.chunkProviderSettings.coalSize);
			}
		}
		...
		if (JurassiCraftConfig.PLANT_GENERATION.mossGeneration) {
			if (BiomeDictionary.hasType(biome, BiomeDictionary.Type.FOREST)
				...
				if (rand.nextInt(8) == 0) {
					...
					if (world.getBlockState(topBlock.down()).isOpaqueCube() && !state.getMaterial().isLiquid()
			...
			if (BiomeDictionary.hasType(biome, BiomeDictionary.Type.SWAMP)
				...
				if (rand.nextInt(8) == 0) {
					...
					if (world.getBlockState(topBlock.down()).isOpaqueCube() && !state.getMaterial().isLiquid()
						...
						world.setBlockState(topBlock, BlockHandler.WEST_INDIAN_LILAC.getDefaultState()
								.withProperty(DoublePlantBlock.HALF, DoublePlantBlock.BlockHalf.LOWER), 2 | 16);
					}
				}
			}
		}
		...
		if (BiomeDictionary.hasType(biome, BiomeDictionary.Type.JUNGLE)) {
			if (rand.nextInt(8) == 0) {
				...
				if (world.getBlockState(topBlock.down()).isOpaqueCube() && !state.getMaterial().isLiquid()
					...
					world.setBlockState(topBlock, BlockHandler.HELICONIA.getDefaultState()
							.withProperty(DoublePlantBlock.HALF, DoublePlantBlock.BlockHalf.LOWER), 2 | 16);
				}
			}
		}
		...
		if (JurassiCraftConfig.PLANT_GENERATION.peatGeneration) {
			if (BiomeDictionary.hasType(biome, BiomeDictionary.Type.SWAMP)) {
				if (rand.nextInt(2) == 0) {
					new WorldGenMinable(BlockHandler.PEAT.getDefaultState(), 5,
							input -> input == Blocks.DIRT.getDefaultState() || input == Blocks.GRASS.getDefaultState())
									.generate(world, rand, world.getTopSolidOrLiquidBlock(pos));
				}
			}
		}
		...
		if (JurassiCraftConfig.MINERAL_GENERATION.trackwayGeneration) {
			...
			if (rand.nextInt(footprintChance) == 0) {
				...
				FossilizedTrackwayBlock.TrackwayType type = FossilizedTrackwayBlock.TrackwayType.values()[rand
						.nextInt(FossilizedTrackwayBlock.TrackwayType.values().length)];

				for (int i = 0; i < rand.nextInt(2) + 1; i++) {
					BlockPos basePos = new BlockPos(pos.getX() + rand.nextInt(10) + 3, y,
							pos.getZ() + rand.nextInt(10) + 3);

					float angle = (float) (rand.nextDouble() * 360.0F);

					IBlockState trackway = BlockHandler.FOSSILIZED_TRACKWAY.getDefaultState()
							.withProperty(FossilizedTrackwayBlock.FACING, EnumFacing.fromAngle(angle))
							.withProperty(FossilizedTrackwayBlock.VARIANT, type);
